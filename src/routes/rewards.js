...
router.get('/rewards-details/:id', isLoggedIn, async (req, res) => {
    const { id } = req.params;
    const rewardList = await pool.query('SELECT R.RewardRewardId, R.RewardName, R.RewardDescription, R.RewardCreatedAt FROM rewards AS R WHERE R.RewardRewardId = ?', [id]);
